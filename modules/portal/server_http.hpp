...
namespace SimpleWeb {
	...
	typedef std::function<boost::future<int>(typename ServerBase<socket_type>::ResponsePtr, std::shared_ptr<typename ServerBase<socket_type>::Request>)> ResourceFunction;
	...
	protected:
		...
		void write_response(std::shared_ptr<socket_type> socket, std::shared_ptr<Request> request, ResourceFunction& resource_function)
		{
			...
			boost::asio::spawn(*strand,
				[this, strand, &resource_function, socket, request, timer](boost::asio::yield_context yield)
				{
